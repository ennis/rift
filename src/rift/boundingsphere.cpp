	...
	: BoundingVolume(context)
{
	...
	// Creating south pole
	meshData.vertices.push_back(glm::vec3(0.0f, -1.0f, 0.0f));
	// Creating north pole
	meshData.vertices.push_back(glm::vec3(0.0f, +1.0f, 0.0f));
	...
	for (int j = 1; j <= nb_lines; j++){
		...
		for (int i = 0; i < nb_vertices_per_line; i++){
			...
			meshData.vertices.push_back(glm::vec3(glm::cos<float>(theta) * glm::cos<float>(phi),
				glm::sin<float>(phi), glm::sin<float>(theta) * glm::cos<float>(phi)));
		}
	}
	...
	for (int k = 0; k < nb_vertices_per_line; k++){
		meshData.indices.push_back(2 + k);
		meshData.indices.push_back(0);
		meshData.indices.push_back(2 + ((k + 1) % nb_vertices_per_line));
	}

	//indices de la coupole nord
	for (int k = 0; k < nb_vertices_per_line; k++){
		meshData.indices.push_back(1);
		meshData.indices.push_back(2 + (nb_lines - 1)*nb_vertices_per_line + k);
		meshData.indices.push_back(2 + (nb_lines - 1)*nb_vertices_per_line + ((k + 1) % nb_vertices_per_line));
	}

	//indices du corps de la sphere
	for (int l = 1; l < nb_lines; l++){
		for (int k = 0; k < nb_vertices_per_line; k++){
			/*sphereIndices[2 + nb_vertices_per_line*(l-1) +k] = 2 + nb_vertices_per_line*(l-1) +k;
			...
			meshData.indices.push_back(2 + nb_vertices_per_line*(l - 1) + k);
			meshData.indices.push_back(2 + nb_vertices_per_line*(l - 1) + ((k + 1) % nb_vertices_per_line));
			meshData.indices.push_back(2 + nb_vertices_per_line* l + k);
			...
			meshData.indices.push_back(2 + nb_vertices_per_line*(l - 1) + ((k + 1) % nb_vertices_per_line));
			meshData.indices.push_back(2 + nb_vertices_per_line*l + ((k + 1) % nb_vertices_per_line));
			meshData.indices.push_back(2 + nb_vertices_per_line* l + k);
		}
	}

	//Definition du submesh
	Submesh sub;
	sub.primitiveType = PrimitiveType::Triangle;
	sub.numIndices = 3 * nb_triangles;
	sub.numVertices = nb_vertices;
	sub.startIndex = 0;
	sub.startVertex = 0;
